...
import { 
  ArrowLeft, Maximize2, RotateCw, Power, Shuffle, 
  ChevronLeft, ChevronRight, Copy, Check, Monitor
} from "lucide-react";
...
export function CompactMode() {
  const { 
    wallpapers, selectedId, setSelectedId, toggleCompactMode 
  } = useAppStore();
  ...
  return (
    <div className="flex flex-col h-full bg-background text-foreground select-none">
      ...
      <div className="flex items-center justify-between p-2 border-b bg-muted/20 drag-region">
        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => toggleCompactMode(false)}>
        ...
        <div className="p-4 space-y-4 flex flex-col items-center">
          ...
          <div className="w-[200px] h-[200px] bg-muted rounded-lg shadow-sm border overflow-hidden relative group">
             ...
             <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                <Button size="icon" variant="secondary" className="rounded-full h-10 w-10" onClick={handleLucky}>
          ...
          <div className="flex items-center gap-2 w-full px-4 justify-center">
             <Button variant="outline" size="icon" className="h-8 w-8 rounded-full" onClick={() => handleNavigate(-1)}>
             ...
             <div className="flex items-center gap-1 bg-muted rounded-md px-2 h-8">
                <Input 
                   ...
                   readOnly
                />
                <span className="text-xs text-muted-foreground">/ {wallpapers.length}</span>
             </div>
          ...
          <div className="w-full space-y-2">
             ...
             <div className="flex flex-wrap gap-1.5">
                <Badge variant="secondary" className="text-[10px] px-1.5 h-5 font-normal">Anime</Badge>
                <Badge variant="secondary" className="text-[10px] px-1.5 h-5 font-normal">Cyberpunk</Badge>
                <Badge variant="secondary" className="text-[10px] px-1.5 h-5 font-normal">4K</Badge>
             </div>
          </div>
      ...
      <div className="p-2 border-t bg-muted/10">
         <div className="flex justify-center gap-2">
            ...
            {[-2, -1, 0, 1, 2].map((offset) => {
               const idx = (currentIndex - 1 + offset + wallpapers.length) % wallpapers.length;
               ...
               return (
                 ...
                 >
                    {wp.preview ? (
                      <img src={convertFileSrc(wp.preview)} className="w-full h-full object-cover" />
                    ) : (
